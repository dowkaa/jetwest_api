...
module.exports = {
    ...
    for (const item of items) {
      ...
      if (parseInt(weight) > volumetric_weight) {
        if (parseFloat(v.available_capacity) - parseFloat(weight) < 0) {
          ...
          Flights.no_of_bags = parseInt(Flights.no_of_bags) + 1;
          await Flights.save();
        }
      } else {
  ...
  getUpdate: async (
    ...
    const totalSuccessfullTransactionsAmount =
      await db.dbs.Transactions.findAll({
        ...
        attributes: [
          [
            util.sequelize.fn("sum", util.sequelize.col("amount_in_dollars")),
            "total_amount",
          ],
        ],
        raw: true,
  ...
  makePayment: async (
    ...
    let checker = await db.dbs.Transactions.findOne({
      where: { amount_in_dollars: amount, shipment_no: shipment_num },
    });
